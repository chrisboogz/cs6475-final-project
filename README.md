...
![frontend example](http://i.imgur.com/TCkjLFB.png)
...
* `/images/:id` - A GET to this endpoint (where `:id` is a specified image ID) returns the image stored at that ID. A DELETE to this endpoint removes the image stored at that ID.
...
* ~~Support for DB storage - It would be really nice to be able to hook my backend up to something like MongoDB and eliminate any potential for out-of-memory issues. However, I didn't want anyone to have to install a full-featured DB simply to play around with my project, so I kept it out for now. A more serious build would definitely use a DB connection, though - perhaps an optional DB connection which could be set in config.~~ I went and did it! The backend now can be configured to connect to a MongoDB instance.
